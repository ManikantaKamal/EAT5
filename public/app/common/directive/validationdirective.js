...
reachApp.directive('ngvalidate', function () {
    ...
    function HideErrorMessagesOnClickorTab(element, domEl) {
        if (element.hasClass('date-required')) {
            if (angular.element(domEl).closest('date-required').next().hasClass("error")) {
                angular.element(domEl).closest('date-required').next().addClass("ng-hide");
                angular.element(domEl).closest('date-required').css("border-color", "#cccccc");
            }
        } else {
            ...
            if (invalid && !element.hasClass("ng-pristine ng-untouched")) {
                ...
                if (angular.element("#" + elementName).hasClass('vmdropdown')) {
                    ...
                    if (!ngDomElem.next().hasClass("error")) {
                        if (element.hasClass('date-required')) {
                            ngDomElem.closest('date-required').css("border-color", "red");
                            ngDomElem.closest('date-required').after('<span class="error">&nbsp;&nbsp;*&nbsp;' + error_msg + '</span>');
                        }
                        ...
                        if (element.hasClass('date-required')) {
                            ngDomElem.closest('date-required').css("border-color", "red");
                            ngDomElem.closest('date-required').next()[0].innerHTML = '&nbsp;&nbsp;*&nbsp;' + error_msg;
                            ngDomElem.closest('date-required').next().removeClass("ng-hide");
                        }
                        else if (ngDomElem.attr('text-angular') != undefined) {
                            ngDomElem.find('.ta-editor').css("border-color", "red");
...
.directive('ngformvalidate', function () {
    ...
    function appendErrorMessageToElement(domEl, ctrl, errmsg) {
        ...
        if (angular.element("#" + ctrl.$name).hasClass('vmdropdown')) {
            ...
            //for input types
            if (!angular.element(domEl.elements[ctrl.$name]).next().hasClass("error")) {
                if (angular.element(ctrl).hasClass('date-required')) {
                    angular.element(domEl.elements[ctrl.$name]).closest('date-required').css("border-color", "red");
                    angular.element(domEl.elements[ctrl.$name]).closest('date-required').after('<span class="error">&nbsp;&nbsp;*&nbsp;' + errmsg + '</span>');
                }
                ...
                if (angular.element(ctrl).hasClass('date-required')) {
                    angular.element(domEl.elements[ctrl.$name]).closest('date-required').css("border-color", "red");
                    angular.element(domEl.elements[ctrl.$name]).closest('date-required').next()[0].innerHTML = '&nbsp;&nbsp;*&nbsp;' + errmsg;
                    angular.element(domEl.elements[ctrl.$name]).closest('date-required').next().removeClass("ng-hide");
                } 
                else if(angular.element(domEl.elements[ctrl.$name]).parent().attr('text-angular')!=undefined)
                {
                    angular.element(domEl.elements[ctrl.$name]).parent().find('.ta-editor').css("border-color", "red");
