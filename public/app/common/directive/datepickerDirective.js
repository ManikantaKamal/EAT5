...
reachApp.directive('vmDatePicker', ['$rootScope', function ($rootScope) {
    return {
        ...
        link: function ($scope, element, attrs, controller) {
            ...
            if ((attrs.datebuttondisabled == undefined || attrs.datebuttondisabled == "false")) { 
                $scope.isDatebuttondisabled = false;
            }
            else  { 
                $scope.isDatebuttondisabled = true;
            }
            
            $scope.dateSelected = function(date){
                if(date !=undefined)
                {
                    if (element.find('span.error') != undefined) {
                        element.find('span.error').addClass("ng-hide");
                        element.find('input.date-required').css("border-color", "#cccccc");
                    }
                }
            }
